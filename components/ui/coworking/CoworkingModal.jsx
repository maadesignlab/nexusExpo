            ...
            <View className="flex-1 justify-end bg-black/50">
                ...
                <View className="bg-white rounded-t-3xl pt-6 px-6 pb-10 max-h-[80%]">
                    ...
                    <View className="flex-row justify-between items-center mb-4">
                        <Text className="text-2xl font-SpaceGroteskBold text-slate-900 flex-1">
                            {space.nombre}
                        </Text>
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        <Text className="text-base text-slate-600 mb-6 font-InterRegular">
                            Este es un espacio de tipo {space.tipo} con una capacidad de {space.capacidad} personas.
                        </Text>

                        {/* Próximos horarios (Simulados) */}
                        <Text className="text-lg font-SpaceGroteskBold text-slate-900 mb-3">
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-8">
                            {['12:00h', '13:00h', '15:00h', '17:00h'].map((time) => (
                                <View key={time} className="mr-3 px-4 py-2 bg-slate-100 border border-slate-200 rounded-xl">
                                    <Text className="text-slate-700 font-InterMedium">{time}</Text>
                                </View>
                            ))}
                        </ScrollView>
                        ...
                        {isOccupied && (
                            <View className="bg-red-50 p-4 rounded-2xl mb-6 border border-red-100">
                                <View className="flex-row items-center mb-1">
                                    ...
                                    <Text className="ml-2 font-InterBold text-red-800">Espacio Ocupado</Text>
                                </View>
                                <Text className="text-red-600 text-sm font-InterRegular">
                    ...
                    <View className="pt-4 border-t border-slate-100">
                        <Pressable
                            ...
                            className={
                                isOccupied 
                                    ? "w-full py-4 rounded-2xl items-center justify-center bg-slate-300"
                                    : "w-full py-4 rounded-2xl items-center justify-center bg-slate-900 active:bg-slate-800"
                            }
                        >
                            <Text className={isOccupied ? "text-lg font-InterBold text-slate-500" : "text-lg font-InterBold text-white"}>
